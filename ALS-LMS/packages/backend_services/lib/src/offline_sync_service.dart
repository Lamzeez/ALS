...
import 'package:backend_services/src/supabase_client.dart';
import 'package:backend_services/src/local/sync_queue_repository.dart';
import 'package:backend_services/src/local/progress_repository.dart';
import 'package:backend_services/src/local/score_repository.dart';
import 'package:backend_services/src/local/course_repository.dart';
import 'package:backend_services/src/local/course_enrollment_repository.dart';
import 'package:backend_services/src/local/course_timeline_repository.dart';
import 'dart:async';
...
class OfflineSyncService {
  ...
  final CourseRepository _courses = CourseRepository();
  final CourseEnrollmentRepository _enrollments = CourseEnrollmentRepository();
  final CourseTimelineRepository _timeline = CourseTimelineRepository();
  ...
  Future<void> _executeOperation(SupabaseClient client, Map<String, dynamic> op) async {
    ...
    switch (table) {
      case DbConstants.tableModuleProgress:
        ...
        break;
      case DbConstants.tableCourseEnrollments:
        // Enrollments use course_id and student_id as composite key
        await client.from(table).upsert(payload, onConflict: 'course_id,student_id');
        break;
      case DbConstants.tableScores:
  ...
  Future<void> trackModuleProgress({
    ...
    await _syncQueue.addToQueue(
      ...
      tableName: DbConstants.tableModuleProgress,
      payload: data,
    );
  ...
  Future<void> enrollInCourse({
    ...
    // 1. Save to local
    await _enrollments.upsert(CourseEnrollment.fromSqlite(data).toSqlite());

    // 2. Add to Sync Queue
    await _syncQueue.addToQueue(
      studentId: studentId,
      operationType: 'upsert',
      tableName: DbConstants.tableCourseEnrollments,
      payload: data,
    );
